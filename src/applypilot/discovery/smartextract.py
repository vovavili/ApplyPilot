...
def _site_key(name: str) -> str:
    """Normalize site names so config matching is forgiving."""
    return re.sub(r"[^a-z0-9]+", "", name.lower())


def select_configured_sites(sites: list[dict], search_cfg: dict) -> list[dict]:
    """Apply the optional smart_extract_sites allowlist from searches.yaml."""
    requested = search_cfg.get("smart_extract_sites")
    if not requested:
        return sites

    allowed = {_site_key(str(name)) for name in requested if str(name).strip()}
    if not allowed:
        return sites
    ...
    found: set[str] = set()
    for site in sites:
        key = _site_key(str(site.get("name", "")))
        if key in allowed:
            selected.append(site)
            found.add(key)

    missing = [str(name) for name in requested if _site_key(str(name)) not in found]
    if missing:
        log.warning("smart_extract_sites entries not found in sites.yaml: %s", ", ".join(missing))
    if not selected:
        log.warning("smart_extract_sites matched no configured boards; skipping smart extract targets.")
...
def run_smart_extract(
    ...
    if configured_sites is None:
        configured_sites = select_configured_sites(load_sites(), search_cfg)

    targets = build_scrape_targets(sites=configured_sites, search_cfg=search_cfg)
    ...
    search_sites = sum(1 for s in configured_sites if s.get("type") == "search")
    static_sites = sum(1 for s in configured_sites if s.get("type") != "search")
    log.info(
